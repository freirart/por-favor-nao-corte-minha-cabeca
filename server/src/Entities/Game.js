...
import defineWinnerUseCase from "../UseCases/defineWinnerUseCase.js";
...
class Game {
  ...
  get summary() {
    return {
      ...
      players: this.players.map((player) => {
        const { name, character, baseScore, killerScore, playerId } = player;
  ...
  canStartNewRound = () => {
    const { currentRound, players, killerIds } = this;
    return (
      !currentRound ||
      (killerIds.length < players.length && !currentRound.canStartANewTurn())
    );
  };

  nextRound = (killerIdParam = "") => {
    const { killerIds, canStartNewRound, rounds } = this;
  ...
  addPlayer(playerName, playerId) {
    if (this.findPlayerByName(playerName) || this.findPlayerById(playerId)) {
      return false;
    }
